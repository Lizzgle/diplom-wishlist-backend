...
public class AcceptFriendRequestHandlerTests
{
    ...
    public AcceptFriendRequestHandlerTests()
    {
        ...
        _handler = new AcceptFriendRequestHandler(_userManagerMock.Object, _friendRequestRepositoryMock.Object, _friendshipRepositoryMock.Object);
    }
    ...
    {
        // Arrange
        var request = new AcceptFriendRequest { Id = Guid.NewGuid(), ReceiverId = "nonexistent-user-id" };
    ...
    public async Task Handle_ShouldThrowNotFoundException_WhenFriendRequestIsNotFound()
    {
        ...
        var request = new AcceptFriendRequest { Id = Guid.NewGuid(), ReceiverId = user.Id };
    ...
    [Fact]
    public async Task Handle_ShouldSuccessfullyAcceptFriendRequest_WhenValid()
    {
        // Arrange
        ...
        // Assert
        _friendRequestRepositoryMock.Verify(m => m.UpdateAsync(It.Is<FriendRequest>(fr => fr.Status == FriendRequestStatus.Accepted), CancellationToken.None), Times.Once);
        _friendshipRepositoryMock.Verify(m => m.AddAsync(It.Is<Friendship>(f => f.Friend1Id == user.Id && f.Friend2Id == sender.Id), CancellationToken.None), Times.Once);
    }
}